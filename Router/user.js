...
app.post("/login", async(req, res) => {
    ...
    let validatePassword = await bcrypt.compare(
        password, Userinfo.password //comparing user entered pass and already exisit hashed pass 
    ) 
    if(!validatePassword){
    ...
    await User.findOneAndUpdate({email}, {$set:{lastLoginDate:date, count}})
...
app.get("/get", async(req, res) => {
    try {
        ...
        if(userinfo.email !== process.env.adminemail){
           return res.status(200).json({ message: "User Details !" , data:userinfo});
        }
